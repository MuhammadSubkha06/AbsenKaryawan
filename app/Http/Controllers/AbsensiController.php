...
class AbsensiController extends Controller
{
  ...
  public function store(Request $request)
  {
    try {
      ...
      if (!$image || $image == 'null') {
        return "error|Foto tidak terdeteksi. Silakan ambil foto ulang.|" . ($jenisAbsen == 'masuk' ? 'in' : 'out');
      }

      // 2. Olah Gambar Base64
      $folderPath = "public/uploads/absensi/";
      $ket = ($jenisAbsen == 'masuk') ? 'masuk' : 'keluar';
      $fileName = $email . "-" . $tanggal . "-" . $ket . ".jpeg";
      ...
      if ($jenisAbsen == 'masuk') {
        ...
        $absenTerakhir = Absen::where('email', $email)
          ->whereNotNull('jam_masuk')
          ->orderBy('id', 'desc')
          ->first();

        if ($absenTerakhir) {
          $update = $absenTerakhir->update([
          ...
          if ($update) {
            Storage::put($folderPath . $fileName, $image_base64);
            return "success|Terimakasih, Selamat beristirahat.|out";
          }
        } else {
          return "error|Data absen masuk tidak ditemukan.|out";
        }
      }

      return "error|Gagal memproses absensi.|" . ($jenisAbsen == 'masuk' ? 'in' : 'out');

    } catch (\Exception $e) {
      // Menangkap detail error database (seperti kolom yang hilang)
